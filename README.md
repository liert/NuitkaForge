...
```bash
python -m nuitka_forge replace app.exe --name module.dll --file ./new_module.dll --out app.patched.exe
```
...
# 深度分析常量资源 payload
python -m nuitka_forge inspect-dll module.dll --constants
```
...
python -m nuitka_forge replace-constants module.dll --dump-section "__parents_main__" -o __parents_main__.json
```
...
4. 内层 DLL 可能包含 Nuitka 常量资源 (RT_RCDATA/3)，并带有 CRC32 校验
